...
if ($_POST){
    ...
    if($pregunta!=null && $respuesta!=null){
        if($existente!=null && $origen==''){
            if ($modificado!='0'){
                modificarPregunta($idItem, $pregunta, $respuesta, $retroalimentacion, 4, null, null, null);
                $noGuardo = 1;
                $redireccionar = 1;
            }
        }else{
            guardarPregunta($id, $pregunta, $respuesta, $retroalimentacion, 4, null, null, null, $posicion);
            $noGuardo = 1;
...
$template = $twig->loadTemplate('pregAbierta.twig.html');
echo $template->render(array("titulo"=>$titulo, 'rol' => $idRol, "pregunta"=>$pregunta, "origen"=>$origen, "volver"=>$redireccionar, 
    ...
    "id"=>$id, "noGuardo"=>$noGuardo, "idItem"=>$idItem, 'ruta'=>$ruta, "modificado"=>$modificado));
?>