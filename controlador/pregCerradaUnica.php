<?php
require_once '../Twig/Autoloader.php'; Twig_Autoloader::register(); $loader = new Twig_Loader_Filesystem('../templates');
$twig = new Twig_Environment($loader, array( 'cache' => 'cache', 'debug' => 'true'));
#Carga de funciones para la conección a la base de datos
include("../bd/funciones.php"); session_start();
...
if ($_POST){
    ...
    if($existente!=null && $modificado==0 && $origen==''){
        $dato = itemRecurso($idItem); $pregunta = $dato['pregunta'];
        $retroalimentacion = $dato['retroalimentacion']; $opciones = opciones($idItem);
    ...
    $pregunta = str_replace("'", " ", $pregunta); $retroalimentacion = str_replace("'", " ", $retroalimentacion);
    $respuesta = str_replace("'", " ", $respuesta); $opciones = str_replace("'", " ", $opciones);
...
$template = $twig->addGlobal("session", $_SESSION); $template = $twig->loadTemplate('pregCerradaUnica.twig.html');
echo $template->render(array("titulo"=>$titulo, "rol"=>$idRol, "pregunta"=>$pregunta, "opciones"=>$opciones, "volver"=>$redireccionar, 
